...
Landscape（风景，人物≤2%）/ Portrait（人像，人物30~50%）/ Environmental Portrait（环境人像，人物约1/3，人与环境共构情绪）/ Still-Life（静物，物体为主角，人物0%）/ Street / Architecture / Wildlife / Action-Moment（对峙、追逐的"前一秒"，人物<5%）。

> ⚠ 静物 vs 技术参考板：本 skill 的 Still-Life 拍的是**有氛围的电影静物**（单件物、氛围光、留白）。若用户要的是"多物件分格、中性背景、均匀布光、形制精确的一致性参考板"（三视图、道具板），那是**技术一致性资产**，不属本 skill——交由 libtv-storyboard 的 reference-assets 直接出 prompt。
...
| 静物 | [templates/still_life.md](templates/still_life.md) |
...
> **静物类型的模块重映射**（人物0%，细则以 `templates/still_life.md` 为准）：Character DNA → 跳过，改写"物体材质身份"（形制/材质/新旧/岁月痕迹，非卖点）；Attention → 从"人物看哪里"重定义为"光落在物体的哪个面/哪条边"（光的注意力=画面最亮处）；Story Moment → 用世界级悬置瞬间（光刚照到刃、尘埃刚落定、物刚被搁下）；其余模块照常。
...
- **静物**：`product catalog, e-commerce packshot, product photography, white seamless background, studio softbox, evenly lit, catalog lighting, ring light, glossy reflection, sparkling highlight, floating object, object centered, hero product shot, advertising, price tag, brand new pristine, game item icon, inventory icon, orthographic reference sheet, multiple objects arranged in grid`
